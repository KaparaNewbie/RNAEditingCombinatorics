...
for col, condition, unique_reads_df in zip(
    ...
    fig.add_annotation(
        ...
        xref="x",
        yref="y",
        text=f"<b>Pearson's r</b><br>p-value = {pv:.2e}<br>ρ = {r:.2f}",
        bgcolor="white",
        borderpad=4,
    )
...
# %%
# (
#     expanded_distinct_unique_proteins_df.query("Fraction == 1.0")
#     .groupby([condition_col, "Protein"])["NumOfReads"]
#     .mean()
#     .max()
# )
...
# x_title = "Reads (mean)"
# y_title = "log<sub>10</sub> compatible unique<br>edited proteins"
# title_text = "Distribution of supporting reads per compatible unique edited protein"
...
# for col, condition in zip(range(1, cols + 1), conditions):
#     df = expanded_distinct_unique_proteins_df.copy()
#     df = (
#         df.loc[(df["Fraction"] == 1.0) & (df[condition_col] == condition)]
#         .groupby([condition_col, "Protein"])["NumOfReads"]
#         .mean()
...
#     df = (
#         df.loc[(df["Fraction"] == 1.0) & (df[condition_col] == condition)]
#         .groupby([condition_col, "Algorithm", "Protein"])["NumOfReads"]
#         .mean()
#         .reset_index()
#     )
#     for i, alg in enumerate(filtering_algs):
...
# %% [markdown]
# #### Coverage - original & assigned expression levels
...
for col, (condition, maximal_df, maximal_solution, maximal_algorithm) in enumerate(zip(conditions, maximal_dfs, maximal_solutions, maximal_algorithms), start=1):
    
    for color, assignment_method, y_col_name in zip(subcolors_discrete_map[condition], assignment_methods, y_col_names):
        
        assignment_df = maximal_df.sort_values(y_col_name, ascending=False).reset_index(drop=True)
...
fig.update_layout(
    ...
    template=template,
    # showlegend=False
    # legend_title_text=f"{condition_col}, Assignment method"
)
...
# %%
X_embedded = TSNE(n_components=2, learning_rate='auto', init='random', perplexity=3, metric=vectors_of_vectors_distances).fit_transform(df.iloc[:4, :7])
...
# %%
df_b.apply(lambda row: row.value_counts(dropna=False))
...
#         mode='markers',
#         marker=dict(
#             color=df["%MeanTotalExpression"],
#             colorscale="Inferno_r",
#             showscale=True,
#             colorbar=dict(
#                 title="% mean expression"
#             ),
#             cmax=df["%MeanTotalExpression"].max(),
#             cmid=df["%MeanTotalExpression"].mean(),
#             cmin=df["%MeanTotalExpression"].min(),
#             line_width=1
#         ),
#         opacity=0.7
#     )
