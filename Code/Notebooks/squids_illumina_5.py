...
# %%
unsorted_primers_for = """
    GCGACTGGTTACCTCAGTGTCAATGC
CAAGACCACCAGCAGCAGCAGC
CCCAGAGTCGTCAGTAGTGGCG
ACGGCCAAGTGTACTTGTCCGG
GCCACCTCCACCGCCGCT
CTCGACTGGTTCCAGGAGTTCC
AAGTCCAGAGTTGATATCATCATCGGT
GTGAGTCCCGCAACAAGGACG
TGTCGCACACACATAACGGATACG
TACGTGTCATACACAGGATTGTCTCTC
CAGCAGTGATCCCGAACGTGGG
CGACAAAGGACCGGAAGGGTG
CCGGGCTGGTGTTGGCACAG
CAATGATGGCATCAGGGGCTC
CATGTGCCCCTGGTTCTGTG
GGCTTGCAGCTCCACGGGATG
GATGCTGTGGGGACACCAGAG
CCACTTCTCAACCACCACTGCC
AGGCTCTAGTCCCACTGGTGTG
ACTGCCACCGATTCCAGCAGAC
    """.split()

unsorted_primers_rev = """
CTGGAAACACCAGGGCAAGACTG
CCAGAATCTGTCATGTTTGGTGGCAC
CAGCATATTTGCACCAGAACTAACTCC
GCGTGAAATCGTTGTCGGAGCG
ACAGACTCAAAGATTGACCAATTGTCAC
CCACCACCTCCCCAGGCT
TGCCAGATGTTGTGGCTGTCAC
CCTGGTTGGTGCTGTCATCACC
GGCCCTGGCTCTCCTTTCGG
CTGAGCAAGTACCCGCTACATCTG
GCGCTGTGACTTGGGATAGGG
CTTCTCCATTGGACCCAAAACTGTAC
GAGATTTTAGTGACCGCCAACGGG
TCCAGACAATTTAGCTCTCCTCGTG
GACATCCTGTAAGGACAAGGTTCC
CGGACGAAGACTTCAACGACTCG
CCAAAGACCTCCGCCAGGTC
GAGGCTTGAGCATTCTGACTCGC
CCTGGGCAATGTCAGAACTAGC
ACCCTGTGGGTTCACATGCAG
""".split()
...
# %%
reads_dfs[0].shape[1] - reads_first_col_pos
...
fig.write_image(
    "Total data loss through the different preprocessing stages - Illumina.svg",
    width=700,
    height=400,
)
...
fig.write_image(
    "Data loss through the different processing stages - Illumina.svg",
    width=1200,
    height=1400,
)
...
for condition, reads_w_nan_df in zip(shortened_conditions, reads_w_nan_dfs):
...
# %%
# for condition, corr, mask in zip(shortened_conditions, corrected_corrs_matrices, fdr_by_masks):
#     fig, ax = plt.subplots(figsize=(11, 9))
...
# %%
# for condition, corr, mask in zip(shortened_conditions, corrected_corrs_matrices, basic_masks):
#     fig, ax = plt.subplots(figsize=(11, 9))
...
merged_noise_df[condition_col] = (
    merged_noise_df[condition_col].str.split("_", expand=True).iloc[:, 0]
)
...
# %%
sampled_concat_meatadata_reads_df = concat_meatadata_reads_df.sample(frac=0.1, random_state=1892)
sampled_concat_meatadata_reads_df[condition_col] = (
    sampled_concat_meatadata_reads_df[condition_col].apply(lambda x: x.split("_")[0])
)
sampled_concat_meatadata_reads_df
...
fig = px.ecdf(
    ...
    color=condition_col,
    color_discrete_map=shortened_color_discrete_map,
    category_orders={condition_col: shortened_conditions},
    template=template,
    labels={
...
fig.update_layout(
    width=1000,
    height=700,
    title_text="Reads sampled to 10%",
)
fig.show()
...
for reads_df, condition in zip(reads_dfs, shortened_conditions):
    nas_per_position_df = (
        pd.DataFrame(
...
fig = px.bar(
    ...
    labels={
        "%NAs": "Reads w/ NAs [%]",
        "RelativePosition%": "Relative editing<br>position [%]",
    }
...
max_distinct_proteins_df[condition_col] = (
    max_distinct_proteins_df[condition_col].str.split("_", expand=True).iloc[:, 0]
)
...
fig.write_image(
    "Max distinct proteins per gene - Illumina.svg",
    width=width,
    height=height,
)
    ...
    if first_data_trace:
        fig.add_trace(
            go.Scatter(
                ...
                legendgroup="Squid's partial-CDS,<br>short-reads",  # this can be any string
                legendgrouptitle_text="Squid's partial-CDS,<br>short-reads",
                # name=condition,
                name=shortened_condition,
            ),
        )
        first_data_trace = False
    else:
...
# %%
max_distinct_proteins_per_transcript_and_alg_df.loc[
    max_distinct_proteins_per_transcript_and_alg_df[condition_col] == "ROBO2_HUMAN"
]
...
fig.write_image("%SolutionsDispersion - Illumina.svg", width=width, height=height)
...
for (
    ...
    train_logspace = [
        int(i)
        for i in np.logspace(
            np.log10(linear_space[0]),
            np.log10(linear_space[1]),
            num=1000 if condition != conditions[6] else 400,
        )
    ]
    # test_logspace = [
    #     int(i)
    ...
    test_logspace = [
        ...
        for i in np.logspace(
            np.log10(linear_space[0]),
            np.log10(linear_space[1]),
            num=800 if condition != conditions[6] else 200,
        )
        if int(i) not in train_logspace
    ]
...
merged_assignment_df.insert(0, "Platform", "Short-reads")
...
robo2_20_recoding_sites.set_axis(
    [f"ROBO2_{i}" for i in range(20)], axis="index"
).to_csv("ROBO2_20_recoding_substitutions.tsv", sep="\t")
robo2_20_recoding_sites
...
fig.write_image("tSNE clustering - Illumina.svg", width=200 * cols, height=200 * rows)
...
fig.write_image(
    "Mean silhouette score of MiniBatchKMeans vs. K size - Illumina.svg",
    width=1000,
    height=800,
)
...
fig.write_image(
    "UMAP clustering & HDBSCAN labeling - Illumina.svg", width=width, height=height
)
...
shannon_df[condition_col] = (
    shannon_df[condition_col].str.split("_", expand=True).iloc[:, 0]
)
...
fig = make_subplots(
    ...
    shared_yaxes="all",
    # x_title="Abundancy",
    y_title="Mean edited positions in originally-supporting reads",
)
...
fig.write_image(
    Path(out_dir, "Mean edited positions vs expression levels - Illumina.svg"),
    width=width,
    height=height,
)
...
subplot_titles = []
for condition in shortened_conditions:
...
fig.write_image(
    Path(
        out_dir,
        "Per-site editing in abundant vs. rare isoforms - IL1.svg"
    ),
    width=width,
    height=height,
)
...
# %%
condition = conditions[0]
reads_df = reads_dfs[0]
reads_df

# %%
one_condition_rare_vs_abundant_isoforms_df = rare_vs_abundant_isoforms_df.loc[
    rare_vs_abundant_isoforms_df[condition_col].eq(condition),
    [condition_col, "Protein", "Reads", "NumOfReads", "Abundancy"]
]

one_condition_rare_vs_abundant_isoforms_df["Reads"] = one_condition_rare_vs_abundant_isoforms_df["Reads"].str.split(",")
assert one_condition_rare_vs_abundant_isoforms_df["Reads"].apply(len).eq(
    one_condition_rare_vs_abundant_isoforms_df["NumOfReads"]
).all()
del one_condition_rare_vs_abundant_isoforms_df["NumOfReads"]
...
for condition, one_condition_rare_vs_abundant_isoforms_df in zip(shortened_conditions, one_condition_rare_vs_abundant_isoforms_dfs):
