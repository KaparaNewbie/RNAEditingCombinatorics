...
# %%
def parasail_pairwise_align(seq1, seq2, matrix=parasail.matrix_create("ACGT", 1, 0)):
    # matrix = parasail.nuc44
    # matrix = parasail.matrix_create("ACGT", 1, 0)
    result = parasail.sw_trace(seq2, seq1, 0, 0, matrix)
...
gaps = sum(
    ...
    if (aligned_ref[i] != aligned_query[i]) and (aligned_ref[i] == '-' or aligned_query[i] == '-')
)
...
# %%
result = parasail.sw_stats_table_scan(seq1, seq2, 0, 0, parasail.nuc44)
print(result)
