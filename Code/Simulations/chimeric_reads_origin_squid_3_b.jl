...
function per_platform_stats_df(
	...
	CSV.write(stats_outfile, stats_df; "\t")
...
results_files = [
	...
	for x_common_proteins in X_common_proteins, y_rare_proteins in Y_rare_proteins
]
...
function plot_stats_df_3(
	...
	for (i, x_common_proteins) in enumerate(X_common_proteins)
		for (j, y_rare_proteins) in enumerate(Y_rare_proteins)
			...
			xs = x_for_row[perm]
			ys = subdf[perm, "%OfChimericProteinsOnAALevel"]
        ...
        linkxaxes!(axes...)
        linkyaxes!(axes...)
        Label(fig[begin:end, 0], "% of chimeric proteins", rotation = pi / 2)
